...
public class General {
    ...
    public String calDuong(int n, int doDainhiPhan) {
        ...
        for (int i = doDainhiPhan; i >= count; i--) {
            output[i] = 0;
        }
        for (int i = doDainhiPhan - 1; i >= 0; i--) {
    ...
    public String calAm(int[] output, int doDainhiPhan) {
        String chuoibit = "";
    ...
    public int chuoiNhiPhanDoDaiToiThieu(int a) {
        int i = 0;
        while (true) {
            if (Math.pow(2, i) > a) {
                  return i;
    ...
    public String ketQuaChuyenDecimaSangBinary(int decima, int doDainhiPhan) {
        if (decima >= 0) {
            return calDuong(decima, doDainhiPhan);
        } else {
            String chuoidaobitstr = calDuong(-decima, doDainhiPhan);
            int[] chuoidaobit = new int[34];
            for (int i = 0; i < doDainhiPhan; i++) {
                chuoidaobit[i] = Integer.parseInt(String.valueOf(chuoidaobitstr.charAt(doDainhiPhan - i)));
            }
            return calAm(chuoidaobit, doDainhiPhan);
        }
    }
